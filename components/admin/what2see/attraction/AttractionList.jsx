...
export default function AttractionList({ pageName }) {
    ...
    function moveToTrash({id}){
        if (confirm("Are you sure want to delete")) {
            db.doc(`${pageName}/${id}`).collection('popularAttraction').get().then((snap)=>{
                snap.forEach((data)=>{
                    db.doc(`${pageName}/${id}`).collection('popularAttraction').doc(`${data.id}`).delete()
                })
            }).then(()=>[
    ...
    const columns = [
        ...
        
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <a onClick={()=>setUpdateDiv(<AttractionUpdate collection={pageName} data={record.itemData}/>)} style={{ color: 'blue' }}><EditFilled/> Edit</a>
    ...
    function ListOfData() {
        return (
            <div >
                <Button style={{margin:"15px 0"}} type='dashed' onClick={()=>setUpdateDiv(<AttractionUpdate collection={pageName} data={undefined}/>)}
