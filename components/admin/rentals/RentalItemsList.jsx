...
import RentalsItemUpdate from './RentalItemUpdate'

export default function RentalsItemList({ collection, id }) {
    ...
    const dataBase = db.doc(`${collection}/${id}`).collection('cabs')
    ...
    const columns = [
        ...
        
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <a onClick={()=>setUpdateDiv(<RentalsItemUpdate collection={`${collection}/${id}`} data={record.itemData}/>)} style={{ color: 'blue' }}><EditFilled/> Edit</a>
    ...
    function ListOfData() {
        return (
            <div >
                <Button style={{margin:"15px 0"}} type='dashed' onClick={()=>setUpdateDiv(<RentalsItemUpdate collection={`${collection}/${id}`} data={undefined}/>)}
                ><PlusOutlined/> Add New Cabs</Button>
                <Table 
                columns={columns} 
